...
=================
Key improvements:
1. SCAN EVERY 15 SECONDS — cuts entry delay from 60s to 15s (free, within Finnhub limits)
2. PRE-MARKET MODE — 6:30-9:30 AM builds watchlist, no alerts sent
   At 9:30 AM open, watchlist already enriched — alerts fire within 15 seconds
3. VOLUME-SCALED POSITION SIZING — scanner returns recommended share count
4. Paper engine + Webull both use scanner's recommended share size
"""
...
class VisionEngine:
    ...
    def _open_paper_positions(self, symbol, price, shares):
        # Internal paper engine
        pos = self.paper_engine.open_position(symbol, price, shares)
        if pos:
            post_paper_open(
                symbol=symbol,
    ...
    def hunt_momentum(self):
        ...
        try:
            ...
            # Prime window: send alerts for top 3
            for stock in top_10[:3]:
                ...
                # Paper positions with volume-scaled share count
                self._open_paper_positions(symbol, price, shares)
    ...
    def start(self):
        ...
        logger.info(
            f"🔥 VISION v18 — scanning every {self.SCAN_INTERVAL_SECONDS}s | "
