...
function renderHistory(){
  ...
  histData.slice(0,60).forEach(t=>{
    ...
    const aType = t.alert_type === 'gap_and_go' ? '⚡ G&G' : '🚩 BF';
    const aColor = t.alert_type === 'gap_and_go' ? 'color:var(--gold)' : 'color:var(--accent)';
    const extBadge = t.extended ? ' ⚠️' : '';
    const hasSnap = t.id && t.chart_snapshot && t.chart_snapshot !== '{}';
    html+=`<div class="hrow">
      <div class="hrow-icon ${iCls}">${ico}</div>
      <div>
        <div class="hrow-sym">$${t.symbol} <span style="font-size:9px;${aColor}">${aType}${extBadge}</span></div>
        <div class="hrow-detail">$${parseFloat(t.entry_price||0).toFixed(2)} · ${et} ET · ${t.shares||0} sh${hasSnap?' · <span style="color:var(--accent);cursor:pointer" onclick="viewSnapshot('+t.id+',\''+t.symbol+'\')">📸 chart</span>':''}</div>
      </div>
...
async function viewSnapshot(tradeId, symbol){
  const data = await fx('/api/trade/'+tradeId+'/snapshot');
  if(!data || !data.snapshot) return;

  const candles = data.snapshot.candles_1min || [];
  const captured = data.snapshot.captured_at || '';
  const alertType = data.alert_type === 'gap_and_go' ? '⚡ Gap and Go' : '🚩 Bull Flag';

  if(candles.length === 0){
    alert('No candle data stored for this trade.');
    return;
  }

  // Build simple ASCII-style chart description
  const prices = candles.map(c => c.c);
  const highs  = candles.map(c => c.h);
  const lows   = candles.map(c => c.l);
  const maxP   = Math.max(...highs).toFixed(2);
  const minP   = Math.min(...lows).toFixed(2);
  const entry  = parseFloat(data.entry_price || 0).toFixed(2);

  // Show in modal overlay
  const modal = document.createElement('div');
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;overflow-y:auto;padding:16px';
  modal.innerHTML = `
    <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;max-width:500px;margin:0 auto;overflow:hidden">
      <div style="padding:12px 14px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
        <div style="font-family:'Syne',sans-serif;font-size:18px;font-weight:700">📸 $${symbol} at Alert</div>
        <button onclick="this.closest('[style*=fixed]').remove()" style="background:none;border:none;color:var(--muted2);font-size:18px;cursor:pointer">✕</button>
      </div>
      <div style="padding:12px 14px">
        <div style="font-size:10px;color:var(--muted2);margin-bottom:8px">${alertType} · Entry $${entry} · Captured ${captured.slice(11,16)} ET</div>
        <div style="font-size:10px;color:var(--muted2);margin-bottom:10px">Range: $${minP} – $${maxP} | Last ${candles.length} 1-min candles at alert time</div>
        <div style="font-family:'JetBrains Mono',monospace;font-size:10px">
          ${candles.map(c=>{
            const isGreen = c.c >= c.o;
            const col = isGreen ? '#00ff87' : '#ff3355';
            const t = String(c.t).slice(11,16);
            const body = Math.abs(c.c - c.o).toFixed(3);
            return `<div style="display:flex;gap:6px;align-items:center;padding:2px 0;border-bottom:1px solid var(--border)">
              <span style="color:var(--muted2);width:40px">${t}</span>
              <span style="color:${col}">${isGreen?'▲':'▼'}</span>
              <span>O:${c.o.toFixed(2)}</span>
              <span>H:${c.h.toFixed(2)}</span>
              <span>L:${c.l.toFixed(2)}</span>
              <span style="color:${col};font-weight:700">C:${c.c.toFixed(2)}</span>
              <span style="color:var(--muted2);font-size:9px">${(c.v/1000).toFixed(0)}k</span>
            </div>`;
          }).join('')}
        </div>
        <div style="margin-top:10px;font-size:9px;color:var(--muted2)">
          Compare this to what actually happened on your Webull chart to evaluate bot timing accuracy.
        </div>
      </div>
    </div>`;
...
function renderAnalytics(){
  ...
  html += breakdownCard('Strategy Type ⚡🚩', anlData.by_alert_type);
...
@app.route('/api/trade/<int:trade_id>/snapshot')
def api_trade_snapshot(trade_id):
    """Return chart snapshot for a specific trade — for backtesting review."""
    try:
        import json
        from database import get_all_trades
        ...
        return jsonify({
            ...
            "alert_type":  trade.get("alert_type", "bull_flag"),
            "snapshot":    snapshot,
        })
    except Exception as e:
...
def api_analytics():
    try:
        ...
        # Add alert_type breakdown
        gap_go = [t for t in trades if t.get("alert_type") == "gap_and_go" and t.get("result")]
        bull   = [t for t in trades if t.get("alert_type") != "gap_and_go" and t.get("result")]
        def wr(lst):
            if not lst: return None
            wins = sum(1 for t in lst if t.get("result") == "win")
            return round(wins / len(lst) * 100, 1)
        report["by_alert_type"] = {
            "Gap and Go ⚡": {"n": len(gap_go), "win_rate": wr(gap_go)},
            "Bull Flag 🚩":  {"n": len(bull),   "win_rate": wr(bull)},
        }
