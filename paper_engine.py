...
# Import DB — graceful fallback if not yet configured
try:
    from database import insert_trade, update_trade_result
    _DB_AVAILABLE = True
except Exception:
    _DB_AVAILABLE = False
    logger.warning("Database not available — trades logged to memory only")
...
class PaperPosition:
    ...
    db_trade_id:   Optional[int] = None  # Supabase row ID for updating result
...
class PaperEngine:
    ...
    def open_position(self, symbol: str, price: float, shares: int = None, **kwargs) -> Optional[PaperPosition]:
        """
        ...
        if _DB_AVAILABLE:
            try:
                trade_id = insert_trade({
                    ...
                    "float_m":        kwargs.get("float_m", 0),
                    "has_news":       kwargs.get("has_news", False),
                    "bull_flag":      kwargs.get("bull_flag", False),
                    "above_vwap":     kwargs.get("above_vwap", False),
                    "above_ema9":     kwargs.get("above_ema9", False),
                    "score":          kwargs.get("score", 0),
                    "hour_of_day":    now.hour,
                    "day_of_week":    now.weekday(),
                    "alert_type":     kwargs.get("alert_type", "bull_flag"),
                    "extended":       kwargs.get("extended", False),
                    "chart_snapshot": kwargs.get("chart_snapshot", {}),
                })
                pos.db_trade_id = trade_id
            except Exception as e:
                logger.warning(f"DB insert failed for {symbol}: {e}")
    ...
    def _close_full(self, pos: PaperPosition, price: float, reason: str):
        ...
        # Update database
        if _DB_AVAILABLE and pos.db_trade_id:
            try:
                result_str = "win" if pos.total_pnl > 0 else ("loss" if pos.total_pnl < 0 else "breakeven")
                update_trade_result(
                    pos.db_trade_id,
                    exit_price=pos.exit_price,
                    pnl_usd=pos.total_pnl,
                    result=result_str,
                    exit_reason=reason,
                )
            except Exception as e:
                logger.warning(f"DB update failed for {pos.symbol}: {e}")
